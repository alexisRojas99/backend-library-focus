...
const swaggerDocument = YAML.load(path.join(__dirname, '../api/docs/index.yaml'));
// Se lee el archivo openapi.YAML que se cargará en /api/docs (swagger publico)
const swaggerDocumentPublic = YAML.load(path.join(__dirname, '../api/docs/index.yaml'));
// Se lee el archivo openapi.YAML para eliminar la propiedad area de los paths.
const swaggerDocumentModify = YAML.load(path.join(__dirname, '../api/docs/index.yaml'));
...
fs.writeFileSync(path.join(__dirname, '../api/docs/temp.yaml'), swaggerTemporal, 'utf8');
...
router.use('/api/doc.json', (req, res) => {
  ...
  res.sendFile(path.join(__dirname, '../api/docs/temp.json'));
});
